...
export default function InterviewConfirmationClient() {
      ...
      <div className="min-h-screen flex flex-col bg-[#f9fdfb]">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-xl p-8 max-w-md mx-4 w-full text-center">
            ...
            <button onClick={loadApplication} className="w-full py-3 rounded-xl bg-[#158a63] text-white font-bold text-sm hover:bg-[#1fae7d] transition-colors">Xem Hồ Sơ</button>
          </div>
        </main>
        <Footer />
      </div>
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col bg-[#f9fdfb]"><Header /><main className="flex-1 flex items-center justify-center"><div className="text-center"><IconLoader2 size={48} className="animate-spin text-[#158a63] mx-auto mb-4" /><p className="text-gray-500 text-sm">Đang tải...</p></div></main><Footer /></div>
    );
  }

  if (error || !application) {
    return (
      <div className="min-h-screen flex flex-col bg-[#f9fdfb]"><Header /><main className="flex-1 flex items-center justify-center"><div className="text-center max-w-md mx-auto px-6"><IconX size={56} className="mx-auto text-red-300 mb-4" /><h2 className="text-xl font-bold text-[#08221a] mb-2">Không Tìm Thấy</h2><p className="text-sm text-gray-500 mb-6">{error || "Không tìm thấy hồ sơ"}</p><Link href="/careers" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#158a63] text-white text-sm font-semibold hover:bg-[#1fae7d] transition-colors">Xem Các Vị Trí Tuyển Dụng<IconArrowRight size={18} /></Link></div></main><Footer /></div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f9fdfb]">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-6 py-12 w-full">
        ...
        {scheduled && (
          <div className="bg-emerald-50 border-2 border-emerald-200 rounded-2xl p-6 text-center mb-6"><IconCheck size={32} className="mx-auto text-[#158a63] mb-2" /><h3 className="font-bold text-[#08221a] mb-1">Đã Đặt Lịch Thành Công!</h3><p className="text-sm text-gray-600">HR sẽ xác nhận lịch phỏng vấn trong thời gian sớm nhất.</p></div>
        )}
        ...
        <div className="text-center mt-8">
          ...
          <div className="flex justify-center gap-4 text-xs"><a href="mailto:tuyendungdaotaovp2@tbsgroup.vn" className="flex items-center gap-1 text-[#158a63] hover:underline"><IconMail size={14} /> tuyendungdaotaovp2@tbsgroup.vn</a><a href="tel:0905359017" className="flex items-center gap-1 text-[#158a63] hover:underline"><IconPhone size={14} /> 0905 359 017</a></div>
        </div>
      </main>
      <Footer />
      <AIChatBubble />

...
router.get("/stats", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (completedTickets.length > 0) {
      const totalResponse = completedTickets.reduce((acc, t) => acc + (t.responseTimeSeconds || 0), 0);
      const totalResolution = completedTickets.reduce((acc, t) => acc + (t.resolutionTimeSeconds || 0), 0);
